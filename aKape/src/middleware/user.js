...
async function mostrarOrdenes() {
  try {
    ...
    console.log("Datos recibidos:", datos);  // Verifica los datos que llegan
    ...
    if (datos.Ordenes && datos.Ordenes.length > 0) {
      datos.Ordenes.forEach((orden) => {
        let Ordenestad = "";  // Declara dentro del ciclo
        ...
        div.innerHTML = `
          <p class="label">Orden #${orden.Idorden}</p>
          <p class="label">Fecha: ${orden.Fechaorden.split("T")[0]}</p>
          <p class="label">Estado: ${Ordenestad}</p>
          <p class="label">Total: $${parseFloat(orden.Total).toFixed(2)}</p>
...
async function orden(Idorden) {
  ...
  // Hacer fetch a la API para obtener la información
  ...
  modal.innerHTML = `
    <center>
      ...
      <h2 class="info">A nombre de ${detalle.Nombre} ${detalle.Paterno} ${detalle.Materno}</h2>
      ...
      <ul>
        ${detalle.productos.map(producto => `
          <li>
            ...
            Comentario de enviado: ${detalle.ComentarioEnviado}<br>
            Comentario de enviado: ${detalle.ComentarioEntregado}<br>
          </li>
        `).join('')}
      </ul>
